...
export class NugetVersions {
	 ...
	 * @param stdOutput
	 */
	public static createFromNugetListAllOutput(
		stdOutput: string,
	): NugetVersions[] {
	...
	public static createFromPath(fullPath: string | undefined): NugetVersions {
		...
		pathParts.some((onePath: string) => {
			const matched: RegExpMatchArray | null =
				PathPartOfVersionRegExp.exec(onePath);
	 ...
	 */
	public static createFromReleasedVersionString(
		releasedVersionString: string,
	): NugetVersions {
	 ...
	 */
	public static createFromFuzzyVersionString(
		fuzzyVersionString: string,
	): NugetVersions {
	...
	public static distance(l: NugetVersions, r: NugetVersions): number {
		const majorDist: number =
			NugetVersions.compareIdentifiers(l.major, r.major) * 1e6;

		if (majorDist !== 0) {
			return majorDist;
		} else {
			const minorDist: number =
				NugetVersions.compareIdentifiers(l.minor, r.minor) * 1e3;
	...
	public static findClosetAmong(
		sortedVersionArr: NugetVersions[],
		expectedVersion: NugetVersions,
	): NugetVersions {
		...
		for (let i: number = 1; i < sortedVersionArr.length; i++) {
			const distance: number = NugetVersions.distance(
				expectedVersion,
				sortedVersionArr[i],
			);
