...
function doResolveRegularSubstitutedValue(valueName: string): string {
	const workspaces: Readonly<vscode.WorkspaceFolder[]> | undefined =
		vscode.workspace.workspaceFolders;

	const workspace: vscode.WorkspaceFolder | undefined = vscode.workspace
	...
	if (Array.isArray(workspaces) && absoluteActivateFilePath) {
		for (const workspace of workspaces) {
			...
			if (
				absoluteActivateFilePath.replace(workspace.uri.fsPath, "") !==
				absoluteActivateFilePath
			) {
...
export function resolveSubstitutedValues(
...
export async function getAnyPqFileBeneathTheFirstWorkspace(): Promise<
	vscode.Uri[]
> {
...
// require-await is not redundant over here
// eslint-disable-next-line require-await
export async function getAnyMProjFilesBeneathTheFirstWorkspace(): Promise<
	vscode.Uri[]
> {
	const theFirstWorkspace: vscode.WorkspaceFolder | undefined =
		getFirstWorkspaceFolder();
...
export function substitutedWorkspaceFolderBasenameIfNeeded(
	str: string
): string {
	const firstWorksapce: vscode.WorkspaceFolder | undefined =
		getFirstWorkspaceFolder();
...
export function manuallyGetLocalVscSetting(
	...
	if (fs.existsSync(expectedVscodeSettingPath)) {
		const content: string = fs.readFileSync(expectedVscodeSettingPath, {
			encoding: "utf8",
		});
...
export function getCurrentWorkspaceSettingPath(): string | undefined {
	const maybeFirstWorkspaceUri: vscode.Uri | undefined =
	...
	if (maybeFirstWorkspaceUri) {
		...
		if (
			expectedVscodeSettingPath &&
			fs.existsSync(expectedVscodeSettingPath)
		) {
...
export function updateCurrentLocalPqModeIfNeeded(
	firstWorkspaceUriFsPath: string
): void {
	// set the language service mode to sdk only if it were defined and also needed for the workspace
	...
	if (!currentLocalPqMode && ExtensionConfigurations.pqMode !== "SDK") {
		void ExtensionConfigurations.setPqMode(
			"SDK",
...
export async function maybeHandleNewWorkspaceCreated(): Promise<void> {
	...
	if (maybeFirstWorkspaceUri) {
		...
		if (fs.existsSync(expectedRootPqPath)) {
			const expectedRootPqPathStat: fs.Stats =
				fs.statSync(expectedRootPqPath);

			// a new workspace just got created
			if (
				Math.abs(expectedRootPqPathStat.ctime.getTime() - Date.now()) <
				6e4
			) {
				// open the expected root-level pq connector file
				void vscode.commands.executeCommand(
					"vscode.open",
					vscode.Uri.file(expectedRootPqPath)
				);
				...
				await vscode.commands.executeCommand(
					"powerquery.sdk.tools.BuildProjectCommand"
				);
