...
export class PqSdkNugetPackageService {
	...
	public async findNullableNewPqSdkVersion(
		...
		if (ExtensionConfigurations.externalsVersionTag === "Recommended") {
			...
			if (!options.maximumNugetVersion) {
				...
				options.maximumNugetVersion =
					this.nullableMaximumPqTestNugetVersion;
			}

			if (!options.minimumNugetVersion) {
				// always restrain the version found above the MinimumPqTestNugetVersion
				options.minimumNugetVersion =
					this.nullableMinimumPqTestNugetVersion;
			}
		} else {
		...
		if (ExtensionConfigurations.nugetPath) {
			sortedNugetVersions =
				await this.nugetCommandService.getSortedPackageReleasedVersions(
			...
			sortedNugetVersions =
				await this.nugetHttpService.getSortedPackageReleasedVersions(
					ExtensionConstants.PublicMsftPqSdkToolsNugetName,
					options
				);
		}
		...
		if (ExtensionConfigurations.externalsVersionTag === "Custom") {
			...
			const expectedNugetVersion: NugetVersions =
				NugetVersions.createFromFuzzyVersionString(
			...
			sortedNugetVersions.length &&
			!sortedNugetVersions[sortedNugetVersions.length - 1].isZero()
		) {
			return sortedNugetVersions[
				sortedNugetVersions.length - 1
			].toString();
		} else {
			return undefined;
		}
	...
	public nugetPqSdkExistsSync(maybeNextVersion?: string): boolean {
		const expectedPqTestPath: string =
			this.expectedPqSdkPath(maybeNextVersion);

		return fs.existsSync(expectedPqTestPath);
	}

	public async updatePqSdkFromNuget(
		maybeNextVersion: string | undefined
	): Promise<string | undefined> {
