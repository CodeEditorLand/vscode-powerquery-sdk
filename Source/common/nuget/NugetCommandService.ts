...
export class NugetCommandService {
	...
	private async doSeizeFromNugetCmd(
		...
		packageName: string = ExtensionConstants.InternalMsftPqSdkToolsNugetName,
		nextVersion: string,
		baseNugetFolder: string = path.resolve(
			this.extensionPath,
			ExtensionConstants.NugetBaseFolder,
		),
	): Promise<string | undefined> {
		const pqTestFullPath: string = this.expectedPackagePath(
			packageName,
			nextVersion,
		);
	...
	public async getPackageReleasedVersions(
		...
		return NugetVersions.createFromNugetListAllOutput(
			await this.doListVersionsFromNugetCmd(
				nugetPath,
				nugetFeed,
				packageName,
				true,
	...
	public async getSortedPackageReleasedVersions(
		...
		let sortedNugetVersions: NugetVersions[] = (
			await this.getPackageReleasedVersions(
				nugetPath,
				nugetFeed,
				packageName,
			)
	...
	public downloadAndExtractNugetPackage(
		...
		return this.doSeizeFromNugetCmd(
			nugetPath,
			nugetFeed,
			packageName,
			packageVersion,
