...
export class NugetHttpService extends NugetLiteHttpService {
	protected override errorHandler: (error: Error) => void = (
		error: Error,
	) => {
	...
	public override async downloadAndExtractNugetPackage(
		...
		this.outputChannel?.appendInfoLine(
			`Start to download ${packageName} ${packageVersion}`,
		);
		...
		await this.downloadNugetPackage(
			...
			this.outputChannel?.appendInfoLine(
				`Nuget package download completed.`,
			);
		});
		...
		setTimeout(async () => {
			try {
				...
				this.outputChannel?.appendErrorLine(
					`Cannot remove ${oneTmpDir} due to ${e}`,
				);
			}
		}, 4e3);
	}
