...
export class NugetLiteHttpService {
		...
		if (nullableHttpProxy) {
			...
			if (nullableHttpProxyAuthHeaderString) {
				this.instance.defaults.headers.common["Authorization"] =
					nullableHttpProxyAuthHeaderString;
			}
		} else {
			this.instance = axios.create({
				baseURL,
	...
	public async getPackageVersions(
		...
		try {
			const response: AxiosResponse<{ versions: string[] }> =
	...
	public async getPackageReleasedVersions(
		...
		const preReleasedVersionIncludeVersions: { versions: string[] } =
			await this.getPackageVersions(packageName);

		preReleasedVersionIncludeVersions.versions =
			preReleasedVersionIncludeVersions.versions.filter(
				(versionStr: string) =>
					NugetLiteHttpService.ReleasedVersionRegex.exec(versionStr),
			);
	...
	public async getSortedPackageReleasedVersions(
		...
		let sortedNugetVersions: NugetVersions[] =
			preReleasedVersionIncludeVersions.versions
				.map((releasedVersion: string) =>
					NugetVersions.createFromReleasedVersionString(
						releasedVersion,
					),
				)
		...
		if (options.maximumNugetVersion) {
			const maximumNugetVersion: NugetVersions = assertNotNull(
		...
		if (options.minimumNugetVersion) {
			const minimumNugetVersion: NugetVersions = assertNotNull(
