...
const InternalActionGetter: (action: CancelAction) => void = (
	_action: CancelAction
) => {
	// noop
};

type InternalHandler = AnyFunction & {
	listener: AnyFunction | { handleEvent: AnyFunction };
};
...
export class CancellationToken implements IVscCancellationToken {
	...
	static isCancellationToken(
		value: { [index: string | symbol]: unknown } | CancellationToken
	): boolean {
		return (
			Boolean(value) &&
			(value as { [index: string | symbol]: unknown })[$$toStringTag] ===
				cancellationTokenTag
		);
	}

	static from(
		value: { [index: string | symbol]: unknown } | AbortSignal
	): CancellationToken {
		if (
			this.isCancellationToken(
				value as { [index: string | symbol]: unknown }
			)
		) {
	...
	get promise(): Promise<Cancel> {
		if (!this._promise) {
			if (this._reason) {
				...
				this._promise = new Promise<Cancel>(
					(resolve: (cancel: Cancel) => void) => {
	...
	public removeEventListener(
		...
		if (this._handlers) {
			const indexToRemove: number = this._handlers.findIndex(
				(internalHandler: AnyFunction) =>
					(internalHandler as InternalHandler).listener === listener
			);
