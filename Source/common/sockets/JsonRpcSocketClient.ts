...
export const defaultBackOff: NumberGenerator = (tries: number = 5) =>
	fibonacciNumbers().addNoise().toMs().take(tries);
...
function makeAsync<T extends AnyFunction>(
	fn: T,
...
export class JsonRpcSocketClient extends SocketClient {
	...
	constructor(
		...
		onMessage: (message: any) => void = defaultOnMessage,
	) {
		super(port, host);
		...
		this.on(CLOSED, () => {
			this.failPendingRequests(
				new SocketConnectionError("Connection has been closed"),
			);
		});
	}
	...
	public exec(rawJsonRpcMessage: Message): Promise<any> {
		if (Message.isResponse(rawJsonRpcMessage)) {
			const rawJsonRpcResponseMessage: ResponseBase =
				rawJsonRpcMessage as ResponseBase;

			if (rawJsonRpcMessage.error) {
				return this._getDeferred(rawJsonRpcResponseMessage.id)?.reject(
					rawJsonRpcResponseMessage.error,
				);
			}

			return this._getDeferred(rawJsonRpcResponseMessage.id)?.resolve(
				rawJsonRpcResponseMessage.result,
			);
		} else if (Message.isNotification(rawJsonRpcMessage)) {
			return this._handle(rawJsonRpcMessage).catch(noop);
		} else {
			return this._handle(rawJsonRpcMessage).then(
				(result: ResponseBase) => result.result,
			);
		}
	}

	public failPendingRequests(reason: any): void {
		const deferredDictionary: Map<number, DeferredJsonRpcTask> =
			this._deferredDictionary;

		deferredDictionary.forEach(
			(deferredJsonRpcTask: DeferredJsonRpcTask) => {
	...
	private _getDeferred(id: number): DeferredJsonRpcTask | undefined {
		const deferred: DeferredJsonRpcTask | undefined =
			this._deferredDictionary.get(id);
