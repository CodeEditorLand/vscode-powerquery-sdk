...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function convertExtensionInfoToLibraryJson(
	...
	for (const oneInfo of extensionInfos) {
		if (oneInfo.Members && Array.isArray(oneInfo.Members)) {
			for (const oneInfoMemeber of oneInfo.Members) {
				...
				if (
					oneInfoMemeber.Documentation &&
					Array.isArray(oneInfoMemeber.Documentation)
				) {
					one.documentation = [];
					...
					for (const oneInfoMemberDoc of oneInfoMemeber.Documentation) {
						one.documentation.push({
							...
							longDescription:
								oneInfoMemberDoc.LongDescription ?? null,
				...
				if (
					oneInfoMemeber.FunctionParameters &&
					Array.isArray(oneInfoMemeber.FunctionParameters)
				) {
					...
					for (const oneInfoMemberPara of oneInfoMemeber.FunctionParameters) {
						one.functionParameters.push({
							...
							sampleValues:
								oneInfoMemberPara.SampleValues ?? null,
							allowedValues:
								oneInfoMemberPara.AllowedValues ?? null,
							defaultValue:
								oneInfoMemberPara.DefaultValue ?? null,
							fields:
								oneInfoMemberPara.Fields &&
								Array.isArray(oneInfoMemberPara.Fields)
									? oneInfoMemberPara.Fields.map(
											(
												oneInfoMemberParaField: FunctionParametersField
											) => ({
												fieldName:
													oneInfoMemberParaField.FiledName,
												type: oneInfoMemberParaField.Type,
												isRequired: Boolean(
													oneInfoMemberParaField.IsRequired
												),
												fieldCaption:
													oneInfoMemberParaField.FieldCaption ??
													null,
												fieldDescription:
													oneInfoMemberParaField.FieldDescription ??
													null,
											})
									  )
									: null,
							...
							enumCaptions:
								oneInfoMemberPara.EnumCaptions ?? null,
						});
					}
				} else {
