...
export class SpawnedProcess {
	...
	constructor(
		...
		this._promise = new Promise<ProcessExit>(
			...
			(
				res: (value: PromiseLike<ProcessExit> | ProcessExit) => void,
				rej: (reason: any) => void,
			) => {
				const theCpStream: ChildProcess = cp.spawn(
					command,
					args ?? [],
					{
						...
						...options,
						stdio: [
							typeof additionalOption?.stdinStr === "string"
								? "pipe"
								: "ignore",
							"pipe",
							"pipe",
