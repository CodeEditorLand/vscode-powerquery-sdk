...
const SimpleBrokerValues = Object.freeze({
	...
	activeColorTheme: new ValueEventEmitter<vscode.ColorTheme>(
...
type SimplePqTestResultViewBrokerValues = ExtractValueEventEmitterTypes<
	typeof SimpleBrokerValues
>;
...
export class SimplePqTestResultViewBroker {
	...
	public static activate(): void {
		...
		for (const oneProperty in this.values) {
			// eslint-disable-next-line security/detect-object-injection, @typescript-eslint/no-explicit-any
			this.values[
				oneProperty as SimplePqTestResultViewBrokerValues
			].subscribe((nextValue: any) => {
				PqTestResultViewPanel.currentPanel?.postOneMessage(
					"OnOneValueUpdated",
					{
	...
	public static getWebviewOptions(
		...
		return {
			...
			localResourceRoots: [
				vscode.Uri.joinPath(
					extensionUri,
					...PqTestResultViewPanel.viewPaths,
				),
			],
		};
	}
