...
import {
	PqServiceHostClient,
	PqServiceHostServerNotReady,
} from "../pqTestConnector/PqServiceHostClient";
import {
	PqTestResultViewPanel,
	SimplePqTestResultViewBroker,
} from "../panels/PqTestResultViewPanel";
import {
	prettifyJson,
	resolveTemplateSubstitutedValues,
} from "../utils/strings";
...
export class LifecycleCommands implements IDisposable {
	...
	constructor(
		...
		globalEventBus.on(
			GlobalEvents.VSCodeEvents.ConfigDidChangeExternalVersionTag,
			() => {
		...
		globalEventBus.on(
			GlobalEvents.VSCodeEvents.ConfigDidChangePqTestVersion,
			() => {
				...
				if (
					...
					this.currentPqTestVersion !==
						ExtensionConfigurations.PQTestVersion
				) {
					void ExtensionConfigurations.setPQTestVersion(
						this.currentPqTestVersion
					);
				} else if (
					ExtensionConfigurations.externalsVersionTag == "Custom" &&
		...
		vscExtCtx.subscriptions.push(
			vscode.commands.registerCommand(
				LifecycleCommands.SeizePqTestCommand,
				this.manuallyUpdatePqTest.bind(this)
			),
			vscode.commands.registerCommand(
			...
			vscode.commands.registerCommand(
				...
				this.commandGuard(this.generateAndSetCredentialCommandV2).bind(
					this
				)
			),
			vscode.commands.registerCommand(
	...
	private activateIntervalTasks(): void {
		...
		this.pqTestService.currentExtensionInfos.subscribe(() => {
			...
			const resolvedPQTestExtensionFileLocation: string | undefined =
				currentPQTestExtensionFileLocation
					? resolveSubstitutedValues(
							currentPQTestExtensionFileLocation
					  )
					: undefined;
			...
			if (
				...
				this.lastCtimeOfMezFileWhoseInfoSeized = getCtimeOfAFile(
					resolvedPQTestExtensionFileLocation
				);

				this.outputChannel.appendInfoLine(
					resolveI18nTemplate(
						"PQSdk.lifecycle.command.update.lastCtimeOfMezFile",
						{
							lastCtimeOfMezFileWhoseInfoSeized: String(
								this.lastCtimeOfMezFileWhoseInfoSeized.getTime()
							),
						}
					)
				);
		...
		this.intervalTaskHandler = setInterval(
			this.intervalTask.bind(this),
			3995
		);
	}
	...
	private promptSettingIncorrectOrInvokeInfoTaskIfNeeded(): void {
		...
		if (
			...
			(!ExtensionConfigurations.featureUseServiceHost ||
				(this.pqTestService as PqServiceHostClient)
					.pqServiceHostConnected)
		) {
			const currentCtime: Date = getCtimeOfAFile(
				resolvedPQTestExtensionFileLocation
			);

			if (
				currentCtime > this.lastCtimeOfMezFileWhoseInfoSeized &&
				...
				if (
					...
					this.onGoingDisplayLatestExtensionInfoCommand.ctime >=
						currentCtime
				) {
					return;
				}
				...
				this.outputChannel.appendInfoLine(
					resolveI18nTemplate(
						"PQSdk.lifecycle.command.detect.newerMezFile",
						{
							currentCtime: String(currentCtime.getTime()),
							diffCtime: String(
								currentCtime.getTime() -
									this.lastCtimeOfMezFileWhoseInfoSeized.getTime()
							),
						}
					)
				...
				this.onGoingDisplayLatestExtensionInfoCommand = {
					ctime: currentCtime,
					deferred: this.displayLatestExtensionInfoCommand(
						currentCtime
					).finally(() => {
						if (
							this.onGoingDisplayLatestExtensionInfoCommand
							...
							this.onGoingDisplayLatestExtensionInfoCommand =
								undefined;
							this.lastCtimeOfMezFileWhoseInfoSeized =
								currentCtime;
						}
					}),
				};
			...
			!this.currentIncorrectConnectorPathInSettingGotPromptedBefore
		) {
			...
			setTimeout(async () => {
				...
				if (
					...
					if (
						anyPqFiles.length &&
						nullableCurrentWorkspaceSettingPath
					) {
						const openStr: string = resolveI18nTemplate(
							"PQSdk.common.open.file",
							{
						...
						const result: string | undefined =
							await vscode.window.showWarningMessage(
								extensionI18n[
									"PQSdk.lifecycle.command.verify.mezFilePath.warning.message"
								],
								openStr,
	...
	private displayLatestExtensionInfoCommand(
		...
		if (
			...
			new Date().getTime() -
				this.currentExecuteTimeOfExtensionDisplayingInfo.getTime() >
				1e4
		) {
	...
	public async promptToSetupCurrentWorkspaceIfNeeded(): Promise<void> {
		const theFirstWorkspace: vscode.WorkspaceFolder | undefined =
			getFirstWorkspaceFolder();

		if (
			theFirstWorkspace &&
			!this.isSuggestingSetupCurrentWorkspace &&
			...
			if (
				...
				const result: string | undefined =
					await vscode.window.showInformationMessage(
						extensionI18n[
							"PQSdk.lifecycle.prompt.update.workspace"
						],
						enableStr,
						extensionI18n["PQSdk.common.cancel"]
					);
	...
	public setupCurrentlyOpenedWorkspaceCommand(): Promise<unknown> {
		...
		if (!hasPQTestExtensionFileLocation) {
			tasks.push(
				(async (): Promise<void> => {
					const mezUrlsBeneathBin: Uri[] =
						await vscWorkspace.findFiles("bin/**/*.{mez}", null, 1);
					...
					if (
						...
						void ExtensionConfigurations.setDefaultExtensionLocation(
							mezExtensionPath
						);

						this.outputChannel.appendInfoLine(
							resolveI18nTemplate(
								"PQSdk.lifecycle.command.set.config",
								{
									configName:
										ExtensionConstants.ConfigNames
											.PowerQuerySdk.properties
											.defaultExtensionLocation,
		...
		if (!ExtensionConfigurations.DefaultQueryFileLocation) {
			tasks.push(
				(async (): Promise<void> => {
					const connectorQueryUrls: Uri[] =
						await vscWorkspace.findFiles("*.{m,pq}", null, 10);

					for (const uri of connectorQueryUrls) {
						const theFSPath: string = uri.fsPath;

						if (
							theFSPath.indexOf(".m") > -1 ||
							theFSPath.indexOf(".query.pq") > -1
						) {
							const relativePath: string =
								vscWorkspace.asRelativePath(uri, false);
							...
							void ExtensionConfigurations.setDefaultQueryFileLocation(
								primaryConnQueryLocation
							);
	...
	private doGenerateOneProjectIntoOneFolderFromTemplates(
		inputFolder: string,
		projectName: string
	): string {
		const folder: string = inputFolder.endsWith(projectName)
			? inputFolder
			: path.join(inputFolder, projectName);

		fs.mkdirSync(folder, { recursive: true });

		const templateTargetFolder: string = path.resolve(
		...
		// copy pngs
		["16", "20", "24", "32", "40", "48", "64", "80"].forEach(
			(onePngSize: string) => {
				fs.copyFileSync(
					path.resolve(
						templateTargetFolder,
						`${templateFileBaseName}${onePngSize}.png`
					),
		...
		[
			...
			let content: string = fs.readFileSync(
				path.resolve(templateTargetFolder, templateFileName),
				{
					encoding: "utf8",
				}
			);
	...
	private async doCheckAndTryToUpdatePqTest(
		skipQueryDialog: boolean = false
	): Promise<string | undefined> {
		try {
			const pqTestLocation: string | undefined =
			...
			if (
				!pqTestLocation ||
				!this.pqTestService.pqTestReady ||
				!this.pqSdkNugetPackageService.nugetPqSdkExistsSync(
					theNextVersion
				)
			) {
				...
				if (!pqTestExecutableFullPath && !skipQueryDialog) {
					const pqTestLocationUrls: Uri[] | undefined =
						await vscode.window.showOpenDialog({
							openLabel:
								extensionI18n[
									"PQSdk.lifecycle.warning.pqtest.required"
								],
							canSelectFiles: true,
				...
				if (pqTestExecutableFullPath) {
					await this.doUpdatePqTestLocationAndStartItIfNeeded(
			...
			void vscode.window.showErrorMessage(
				resolveI18nTemplate(
					"PQSdk.lifecycle.command.update.sdkTool.errorMessage",
					{
						errorMessage,
					}
				)
	 ...
	 * @private
	 */
	private checkAndTryToUpdatePqTest(
		skipQueryDialog: boolean = false
	): Promise<string | undefined> {
		if (!this.checkAndTryToUpdatePqTestDeferred$) {
			this.checkAndTryToUpdatePqTestDeferred$ =
				this.doCheckAndTryToUpdatePqTest(skipQueryDialog);
		}
		...
		try {
			...
			if (
				...
				const pqTestExecutableFullPath: string =
					this.pqSdkNugetPackageService.expectedPqSdkPath(
						maybeNextVersion
					);

				this.outputChannel.appendInfoLine(
					resolveI18nTemplate(
						"PQSdk.lifecycle.command.pqtest.seized.from",
						{
							pqTestExecutableFullPath,
						}
					)
				...
				this.outputChannel.appendErrorLine(
					extensionI18n[
						"PQSdk.lifecycle.warning.pqtest.seized.failed"
					]
				);
			}
			...
			if (pqTestLocation) {
				this.outputChannel.appendInfoLine(
					resolveI18nTemplate(
						"PQSdk.lifecycle.command.pqtest.set.to",
						{
							pqTestLocation:
								ExtensionConfigurations.PQTestLocation,
						}
				...
				this.outputChannel.appendErrorLine(
					extensionI18n["PQSdk.lifecycle.warning.pqtest.set.failed"]
				);
			}
			...
			void vscode.window.showErrorMessage(
				resolveI18nTemplate(
					"PQSdk.lifecycle.command.manuallyUpdate.sdkTool.errorMessage",
					{
						errorMessage,
					}
				)
	...
	public debouncedManuallyUpdatePqTest: (
		maybeNextVersion?: string
	) => Promise<string | undefined> = debounce(
		(maybeNextVersion?: string) =>
			this.manuallyUpdatePqTest(maybeNextVersion),
		2e3
	).bind(this) as typeof this.manuallyUpdatePqTest;

	public async generateOneNewProject(): Promise<void> {
		const newProjName: string | undefined =
			await vscode.window.showInputBox({
				title: extensionI18n[
					"PQSdk.lifecycle.command.new.project.title"
				],
				placeHolder:
					extensionI18n[
						"PQSdk.lifecycle.command.new.project.placeHolder"
					],
				validateInput(
					value: string
				):
					...
					if (!value) {
						return extensionI18n[
							"PQSdk.lifecycle.error.empty.project.name"
						];
					} else if (!value.match(validateProjectNameRegExp)) {
						return extensionI18n[
							"PQSdk.lifecycle.error.invalid.project.name"
						];
					}
		...
		if (newProjName) {
			const firstWorkspaceFolder: WorkspaceFolder | undefined =
				getFirstWorkspaceFolder();

			if (firstWorkspaceFolder) {
				// we got the workspace and let's generate files into the first workspace
				const targetFolder: string =
					this.doGenerateOneProjectIntoOneFolderFromTemplates(
				...
				if (targetFolder === firstWorkspaceFolder.uri.fsPath) {
					...
					await vscode.commands.executeCommand(
						"vscode.open",
						vscode.Uri.file(
							path.join(targetFolder, `${newProjName}.pq`)
						)
					);

					void vscode.window.showInformationMessage(
						resolveI18nTemplate(
							"PQSdk.lifecycle.command.new.project.created",
							{
								newProjName,
								targetFolder,
							}
				...
				const selectedFolders: Uri[] | undefined =
					await vscode.window.showOpenDialog({
						canSelectMany: false,
						openLabel:
							extensionI18n[
								"PQSdk.lifecycle.command.select.workspace"
							],
	...
	public async deleteCredentialCommand(): Promise<void> {
		await vscode.window.withProgress(
			{
				title: extensionI18n[
					"PQSdk.lifecycle.command.delete.credentials.title"
				],
			...
			async (
				...
				try {
					const result: GenericResult =
						await this.pqTestService.DeleteCredential();

					this.outputChannel.appendInfoLine(
						resolveI18nTemplate(
							"PQSdk.lifecycle.command.delete.credentials.result",
							{
					...
					void vscode.window.showErrorMessage(
						resolveI18nTemplate(
							"PQSdk.lifecycle.command.delete.credentials.errorMessage",
							{
								errorMessage,
							}
						)
	...
	public async displayExtensionInfoCommand(): Promise<void> {
		await vscode.window.withProgress(
			{
				title: extensionI18n[
					"PQSdk.lifecycle.command.display.extension.info.title"
				],
				location: ProgressLocation.Window,
				cancellable: true,
			},
			async (
				...
				try {
					...
					this.outputChannel.appendInfoLine(
						resolveI18nTemplate(
							"PQSdk.lifecycle.command.display.extension.info.result",
							{
								result: result
									.map(
										(info: ExtensionInfo) => info.Name ?? ""
									)
									.filter(Boolean)
									.join(","),
							}
					...
					if (
						...
						void vscode.window.showErrorMessage(
							resolveI18nTemplate(
								"PQSdk.lifecycle.command.display.extension.info.errorMessage",
								{
									errorMessage,
								}
							)
	...
	public async listCredentialCommand(): Promise<void> {
		await vscode.window.withProgress(
			{
				title: extensionI18n[
					"PQSdk.lifecycle.command.list.credentials.title"
				],
				location: ProgressLocation.Window,
				cancellable: true,
			},
			async (
				...
				try {
					const result: unknown[] =
						await this.pqTestService.ListCredentials();

					this.outputChannel.appendInfoLine(
						resolveI18nTemplate(
							"PQSdk.lifecycle.command.list.credentials.result",
							{
								result: prettifyJson(result),
							}
						)
					...
					void vscode.window.showErrorMessage(
						resolveI18nTemplate(
							"PQSdk.lifecycle.command.list.credentials.errorMessage",
							{
								errorMessage,
							}
						)
	...
	private async doPopulateOneSubstitutedValue(
		...
		const valueKey: string | undefined = await vscode.window.showInputBox({
			...
			validateInput(
				value: string
			): string | Thenable<string | undefined | null> | undefined | null {
				if (!value) {
					return resolveI18nTemplate(
						"PQSdk.lifecycle.error.invalid.empty.value",
						{
							valueName,
						}
					);
		...
		switch (theAuthenticationKind) {
			...
			case "OAuth":
				...
				templateStr = await this.doPopulateOneSubstitutedValue(
					templateStr,
					extensionI18n[
						"PQSdk.lifecycle.credential.accessToken.label"
					],
					"$$ACCESS_TOKEN$$"
				);
				...
				templateStr = await this.doPopulateOneSubstitutedValue(
					templateStr,
					extensionI18n[
						"PQSdk.lifecycle.credential.refreshToken.label"
					],
					"$$REFRESH_TOKEN$$"
				);
	...
	public async generateAndSetCredentialCommand(): Promise<void> {
		await vscode.window.withProgress(
			{
				title: extensionI18n[
					"PQSdk.lifecycle.command.generate.credentials.title"
				],
				location: ProgressLocation.Window,
				cancellable: true,
			},
			async (
				...
				try {
					...
					const credentialPayload: any =
						await this.pqTestService.GenerateCredentialTemplate();

					this.outputChannel.appendInfoLine(
						resolveI18nTemplate(
							"PQSdk.lifecycle.command.generate.credentials.result",
							{
					...
					const credentialPayloadStr: string =
						await this.populateCredentialTemplate(
							credentialPayload
						);
					// eslint-disable-next-line @typescript-eslint/no-explicit-any
					const result: any =
						await this.pqTestService.SetCredential(
							credentialPayloadStr
						);

					this.outputChannel.appendInfoLine(
						resolveI18nTemplate(
							"PQSdk.lifecycle.command.set.credentials.result",
							{
								result: prettifyJson(result),
							}
						)
					...
					void vscode.window.showInformationMessage(
						resolveI18nTemplate(
							"PQSdk.lifecycle.command.set.credentials.info",
							{
								authenticationKind:
									credentialPayload.AuthenticationKind,
							}
						)
					);
					// eslint-disable-next-line @typescript-eslint/no-explicit-any
					...
					void vscode.window.showErrorMessage(
						resolveI18nTemplate(
							"PQSdk.lifecycle.command.set.credentials.errorMessage",
							{
								errorMessage,
							}
						)
	 ...
	 */
	public validateCreateAuthState(
		...
		if (
			...
			return extensionI18n[
				"PQSdk.lifecycle.error.invalid.missing.dataSourceKindAndAuthKind"
			];
		}

		if (
			createAuthState.AuthenticationKind.toLowerCase() ===
				"usernamepassword" &&
			(!createAuthState.$$PASSWORD$$ || !createAuthState.$$USERNAME$$)
		) {
			return resolveI18nTemplate(
				"PQSdk.lifecycle.error.invalid.missing.userNameAndPw",
				{
		...
		if (
			createAuthState.AuthenticationKind.toLowerCase() === "key" &&
			!createAuthState.$$KEY$$
		) {
			return resolveI18nTemplate(
				"PQSdk.lifecycle.error.invalid.missing.key",
				{
					authenticationKind: createAuthState.AuthenticationKind,
				}
			);
	...
	public async generateAndSetCredentialCommandV2(): Promise<void> {
		const title: string =
			extensionI18n["PQSdk.lifecycle.command.generate.credentials.title"];
		...
		await vscode.window.withProgress(
			...
			async (
				...
				try {
					...
					const authenticationKindMap: Map<
						string,
						Set<string>
					> = new Map();

					currentExtensionInfos.forEach(
							...
							currentExtensionInfo.DataSources.forEach(
								(
									...
									const currentSetOfTheDataSource: Set<string> =
										authenticationKindMap.get(
											oneDataSource.DataSourceKind
										) ?? new Set();

									if (
										!authenticationKindMap.has(
											oneDataSource.DataSourceKind
										)
									) {
										authenticationKindMap.set(
											oneDataSource.DataSourceKind,
											currentSetOfTheDataSource
										);
									}
					...
					const connectorQueryFiles: vscode.Uri[] =
					...
					async function populateDataSourceKinds(
						...
						if (dataSourceKinds.length) {
							const items: vscode.QuickPickItem[] =
								dataSourceKinds.map((one: string) => ({
									label: one,
								}));

							items.push({
								label: extensionI18n[
									"PQSdk.lifecycle.command.choose.customizedDataSourceKind.label"
								],
							});
							...
							const picked: vscode.QuickPickItem =
								await input.showQuickPick({
									...
									placeholder:
										extensionI18n[
											"PQSdk.lifecycle.command.choose.dataSourceKind"
										],
									activeItem: items[0],
									items,
								});
						...
						if (
							...
							state.DataSourceKind = await input.showInputBox({
								...
								totalSteps: 3,
								value: "",
								prompt: extensionI18n[
									"PQSdk.lifecycle.command.choose.dataSourceKind.label"
								],
								ignoreFocusOut: true,
								validate: (key: string) =>
									Promise.resolve(
										key.length
											? undefined
											: extensionI18n[
													"PQSdk.lifecycle.error.empty.dataSourceKind"
											  ]
									),
							});
						}
					...
					async function populateQueryFile(
						...
						if (connectorQueryFiles.length) {
							const items: vscode.QuickPickItem[] =
								connectorQueryFiles.map((one: vscode.Uri) => ({
							...
							items.push({
								label: extensionI18n[
									"PQSdk.lifecycle.command.choose.customizedQueryFilePath.label"
								],
								detail: extensionI18n[
									"PQSdk.lifecycle.command.choose.customizedQueryFilePath.detail"
								],
							});
							...
							const picked: vscode.QuickPickItem =
								await input.showQuickPick({
									...
									placeholder:
										extensionI18n[
											"PQSdk.lifecycle.command.choose.queryFile"
										],
									activeItem: items[0],
									items,
								});
						...
						if (
							...
							state.PathToQueryFile = await input.showInputBox({
								...
								value: "",
								prompt: extensionI18n[
									"PQSdk.lifecycle.command.choose.queryFilePath.label"
								],
								ignoreFocusOut: true,
								validate: (key: string) =>
									Promise.resolve(
										key.length
											? undefined
											: extensionI18n[
													"PQSdk.lifecycle.error.empty.PathToQueryFilePath"
											  ]
									),
							});
							...
							state.PathToConnectorFile = path.dirname(
								state.PathToQueryFile
							);

							// need to populate the PathToConnectorFile to re-verify
							// eslint-disable-next-line require-atomic-updates
							state.PathToConnectorFile =
								await input.showInputBox({
									...
									prompt: extensionI18n[
										"PQSdk.lifecycle.command.choose.extensionFilePath.label"
									],
									ignoreFocusOut: true,
									validate: (key: string) =>
										Promise.resolve(
											key.length
												? undefined
												: extensionI18n[
														"PQSdk.lifecycle.error.empty.PathToConnectorFile"
												  ]
										),
								});
						}
						...
						return (input: MultiStepInput) =>
							pickAuthenticationKind(input, state);
					}
					...
					async function pickAuthenticationKind(
						...
						let currentAuthCandidates: string[] =
							state.DataSourceKind
								? Array.from(
										authenticationKindMap.get(
											state.DataSourceKind
						...
						const items: vscode.QuickPickItem[] =
							currentAuthCandidates.map((one: string) => ({
								label:
									one.toLowerCase() === "implicit"
										? "Anonymous"
										: one,
							}));
						...
						const picked: vscode.QuickPickItem =
							await input.showQuickPick({
								...
								placeholder:
									extensionI18n[
										"PQSdk.lifecycle.command.choose.auth"
									],
								activeItem: items[0],
								items,
							});
						...
						if (state.AuthenticationKind.toLowerCase() === "key") {
							return (input: MultiStepInput) =>
								populateKey(input, state);
						} else if (
							state.AuthenticationKind.toLowerCase() ===
							"usernamepassword"
						) {
							return (input: MultiStepInput) =>
								populateUsername(input, state);
						}
					}
					...
					async function populateKey(
						...
						state.$$KEY$$ = await input.showInputBox({
							...
							value: "",
							prompt: extensionI18n[
								"PQSdk.lifecycle.command.choose.authKind.prompt"
							],
							ignoreFocusOut: true,
							validate: (key: string) =>
								Promise.resolve(
					...
					async function populateUsername(
						...
						state.$$USERNAME$$ = await input.showInputBox({
							...
							value: "",
							prompt: extensionI18n[
								"PQSdk.lifecycle.credential.username.label"
							],
							ignoreFocusOut: true,
					...
					async function populatePassword(
						...
						state.$$PASSWORD$$ = await input.showInputBox({
							...
							ignoreFocusOut: true,
							prompt: extensionI18n[
								"PQSdk.lifecycle.credential.password.label"
							],
							password: true,
							validate: (_pw: string) =>
					...
					if (maybeErrorMessage) {
						...
						try {
							const result: GenericResult =
								await this.pqTestService.SetCredentialFromCreateAuthState(
									createAuthState
								);

							this.outputChannel.appendInfoLine(
								resolveI18nTemplate(
									"PQSdk.lifecycle.command.createAuthState.result",
									{
										result: prettifyJson(result),
									}
								)
							...
							void vscode.window.showErrorMessage(
								resolveI18nTemplate(
									"PQSdk.lifecycle.command.createAuthState.ofKind.errorMessage",
									{
										authenticationKind:
											createAuthState.AuthenticationKind,
										errorMessage,
					...
					void vscode.window.showErrorMessage(
						resolveI18nTemplate(
							"PQSdk.lifecycle.command.createAuthState.errorMessage",
							{
								errorMessage,
							}
						)
	...
	public async refreshCredentialCommand(): Promise<void> {
		await vscode.window.withProgress(
			...
			async (
				...
				try {
					const result: GenericResult =
						await this.pqTestService.RefreshCredential();

					this.outputChannel.appendInfoLine(
						resolveI18nTemplate(
							"PQSdk.lifecycle.command.refresh.credentials.result",
							{
								result: prettifyJson(result),
							}
						)
					...
					void vscode.window.showErrorMessage(
						resolveI18nTemplate(
							"PQSdk.lifecycle.command.refresh.credentials.errorMessage",
							{
								errorMessage,
							}
						)
			...
			async (
				...
				try {
					if (ExtensionConfigurations.featureUseServiceHost) {
						result = await (
							this.pqTestService as PqServiceHostClient
						).RunTestBatteryFromContent(pathToQueryFile?.fsPath);
					} else {
						result = await this.pqTestService.RunTestBattery(
					...
					this.outputChannel.appendInfoLine(
						resolveI18nTemplate(
							"PQSdk.lifecycle.command.run.test.result",
							{
								result: prettifyJson(result),
							}
						)
					...
					void vscode.window.showErrorMessage(
						resolveI18nTemplate(
							"PQSdk.lifecycle.command.run.test.errorMessage",
							{
								errorMessage,
							}
						)
	...
	public async testConnectionCommand(): Promise<void> {
		await vscode.window.withProgress(
			{
				title: extensionI18n[
					"PQSdk.lifecycle.command.test.connection.title"
				],
				location: ProgressLocation.Window,
				cancellable: true,
			},
			async (
				...
				try {
					const result: GenericResult =
						await this.pqTestService.TestConnection();

					this.outputChannel.appendInfoLine(
						resolveI18nTemplate(
							"PQSdk.lifecycle.command.test.connection.result",
							{
								result: prettifyJson(result),
							}
						)
					...
					void vscode.window.showErrorMessage(
						resolveI18nTemplate(
							"PQSdk.lifecycle.command.test.connection.errorMessage",
							{
								errorMessage,
							}
						)
