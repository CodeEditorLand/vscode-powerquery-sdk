...
function createExtensionI18nRecord(): ExtensionI18nRecord {
	return new Proxy<ExtensionI18nRecord>(defaultJson, {
		get(
			target: ExtensionI18nRecord,
			property: ExtensionI18nKeys,
			_receiver: unknown,
		): string {
...
export function handleLocaleChanged(nextLocale?: string): void {
	...
	if (currentLocale !== nextLocale && fs.existsSync(expectedLocalJsonPath)) {
		const expectedLocaleContent: string = fs.readFileSync(
			expectedLocalJsonPath,
			{ encoding: "utf-8" },
		);
...
function doResolveI18nTemplate<R extends Record<string, string>>(
	...
	while (curMatch) {
		...
		if (
			theMatchedArgumentName &&
			argumentsPackage[theMatchedArgumentName]
		) {
			theReplacedStr =
				argumentsPackage[theMatchedArgumentName] ??
				theMatchedArgumentName;
...
export function resolveI18nTemplate(
	...
	return doResolveI18nTemplate<ExtensionI18nRecord>(
		extensionI18n,
		i18nKey,
		argumentsPackage,
	);
