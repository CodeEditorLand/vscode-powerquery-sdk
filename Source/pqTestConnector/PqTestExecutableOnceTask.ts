...
type PqTestExecutableOnceTaskEventTypes = ExtractEventTypes<
	typeof PqTestExecutableOnceTaskQueueEvents
>;
...
export class PqTestExecutableOnceTask implements IDisposable {
	...
	private handleTaskCreated(): void {
		setTimeout(() => {
			this.eventBus.emit(
				PqTestExecutableOnceTaskQueueEvents.processCreated,
			);
		}, 0);
	}
	...
	private handleTaskExited(): void {
		setTimeout(() => {
			this.eventBus.emit(
				PqTestExecutableOnceTaskQueueEvents.processExited,
			);
		}, 0);
	}
	...
	private get pqTestFullPath(): string {
		...
		const pqtestExe: string = path.resolve(
			nextPQTestLocation,
			PqTestExecutableOnceTask.ExecutableName,
		);
	...
	private populateTestTaskPayload(
		program: string,
		task: PQTestTask,
	): PQTestTask {
	...
	public async run(program: string, task: PQTestTask): Promise<void> {
		try {
			...
			const spawnProcess: SpawnedProcess = new SpawnedProcess(
				...
				{ cwd: path.dirname(pqTestExeFullPath) },
				{
					...
					onSpawned: (childProcess: ChildProcess): void => {
						...
						this.handleOutputStr(
							resolveI18nTemplate(
								"PQSdk.taskQueue.info.debugTaskBegan",
								{
			...
			if (
				...
				this.handleErrorStr(
					resolveI18nTemplate(
						"PQSdk.taskQueue.info.debugTaskExitAbnormally",
						{
							operation: task.operation,
