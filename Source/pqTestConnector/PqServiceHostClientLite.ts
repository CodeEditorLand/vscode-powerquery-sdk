...
import {
	defaultBackOff,
	JsonRpcSocketClient,
} from "../common/sockets/JsonRpcSocketClient";
	...
	implements Omit<IPQTestService, OmittedPqTestMethods>, IDisposable
{
	...
	public async requestRemoteRpcMethod<
	...
	>(
		...
		if (this.jsonRpcSocketClient) {
			const responseResult: PqServiceHostResponseResult =
				(await this.jsonRpcSocketClient.request(
					method,
					parameters
					// eslint-disable-next-line @typescript-eslint/no-explicit-any
			...
			if (responseResult.Status === ResponseStatus.Success) {
				if (
					options.shouldParsePayload &&
					typeof responseResult.Payload === "string"
				) {
					try {
				...
				// eslint-disable-next-line @typescript-eslint/no-explicit-any
				const errorData: any =
					responseResult.InnerException ?? responseResult.Payload;

				const errorMessage: string = getInternalErrorMessage(errorData);

				return Promise.reject(
					new PqInternalError(errorMessage, errorData)
				);
			}
		} else {
	...
	private async createJsonRpcSocketClient(port: number): Promise<void> {
		...
		try {
			...
			this.outputChannel.appendErrorLine(
				`Failed to listen PqServiceHost.exe at ${port} due to ${error}`
			);
		}

		if (theJsonRpcSocketClient.status === OPEN) {
			this.outputChannel.appendInfoLine(
				`Succeed listening PqServiceHost.exe at ${port}`
			);
	...
	private resolvePQServiceHostPath(
		nextPQTestLocation: string | undefined
	): string | undefined {
		if (!nextPQTestLocation) {
			this.outputChannel.appendErrorLine(
				"powerquery.sdk.tools.location configuration value is not set."
			);
		...
		const pqServiceHostExe: string = path.resolve(
			nextPQTestLocation,
			PqServiceHostClientLite.ExecutableName
		);

		if (!fs.existsSync(pqServiceHostExe)) {
			this.outputChannel.appendErrorLine(
				`PqServiceHost.exe not found at ${pqServiceHostExe}`
			);
	...
	private disposeCurrentJsonRpcSocketClient(): void {
		if (this.jsonRpcSocketClient) {
			...
			this.outputChannel.appendInfoLine(
				`Stop listening PqServiceHost.exe`
			);
		}
	}

	private doSeizeNumberFromLockFile(
		theLockFileFullPath: string
	): number | undefined {
		...
		const pidString: string = fs
			.readFileSync(theLockFileFullPath)
			.toString("utf8");
	...
	private async doStartAndListenPqServiceHostIfNeeded(
		...
		try {
			...
			let pidNumber: number | undefined =
				this.doSeizeNumberFromLockFile(pidFileFullPath);
			...
			if (
				...
				this.outputChannel.appendInfoLine(
					`#${tryNumber + 1} try to boot PqServiceHost.exe`
				);
			}
			...
			while (maxTry > 0 && !portInUse) {
				...
				this.outputChannel.appendInfoLine(
					`Check #[${
						5 - maxTry
					}] whether PqServiceHost.exe exported at ${portNumber}, ${portInUse}`
				);
	...
	DeleteCredential(): Promise<GenericResult> {
		return this.requestRemoteRpcMethod(
			"v1/PqTestService/DeleteCredential",
			[
				{
	...
	RefreshCredential(): Promise<GenericResult> {
		return this.requestRemoteRpcMethod(
			"v1/PqTestService/RefreshCredential",
			[
				{
					SessionId: this.sessionId,
					PathToConnector: getFirstWorkspaceFolder()?.uri.fsPath,
	...
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	async RunTestBatteryFromContent(
		...
		if (!pathToQueryFile || !fs.existsSync(pathToQueryFile))
			return Promise.resolve();

		let currentContent: string = fs.readFileSync(pathToQueryFile, {
			encoding: "utf8",
		});

		vscode.window.visibleTextEditors.forEach(
			(oneEditor: vscode.TextEditor) => {
				if (
	...
	SetCredentialFromCreateAuthState(
