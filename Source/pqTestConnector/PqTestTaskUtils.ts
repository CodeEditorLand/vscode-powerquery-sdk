...
import {
	getAnyMProjFilesBeneathTheFirstWorkspace,
...
export async function executeBuildTaskAndAwaitIfNeeded(
	...
	lastPqRelatedFileTouchedDateUpdater: (
	...
	if (nullableCurrentWorkspace) {
		...
		for await (const oneFullPath of globFiles(
			path.join(nullableCurrentWorkspace, "bin"),
			(fullPath: string) => fullPath.endsWith(".mez")
		)) {
			currentlyAllMezFiles.push(oneFullPath);
		}

		if (currentlyAllMezFiles.length === 1) {
			const theCtimeOfTheFile: Date = getCtimeOfAFile(
				currentlyAllMezFiles[0]
			);
			needToRebuildBeforeEvaluation =
				theCtimeOfTheFile <= lastPqRelatedFileTouchedDate;
		} else {
		...
		if (needToRebuildBeforeEvaluation) {
			...
			let theBuildTask: vscode.Task =
				PowerQueryTaskProvider.buildMakePQXCompileTask(pqTestLocation);

			// check if we had any legacy *.mproj or .proj beneath the first directory
			const anyMProjFilesFromLegacyProjects: vscode.Uri[] =
				await getAnyMProjFilesBeneathTheFirstWorkspace();

			if (
				anyMProjFilesFromLegacyProjects.length &&
				(ExtensionConfigurations.msbuildPath ||
					findExecutable("msbuild", [".exe", ""]))
			) {
...
export function inferAnyGeneralErrorString(resultJson: any): string {
	...
	if (Array.isArray(resultJson)) {
		...
		typeof resultJson["ErrorStatus"] === "string" &&
		resultJson["ErrorStatus"]
	) {
...
export function formatArguments(args: string[]): string {
	...
	for (const oneArg of args) {
		// pre formatting
		const isCurrentArgumentParameter: boolean =
			oneArg.indexOf("--") === 0 || oneArg.indexOf("-") === 0;
