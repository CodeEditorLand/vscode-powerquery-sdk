...
import {
	...
	ExtractEventTypes,
} from "../common/DisposableEventEmitter";
import {
	executeBuildTaskAndAwaitIfNeeded,
	formatArguments,
	inferAnyGeneralErrorString,
} from "./PqTestTaskUtils";
...
type PqTestExecutableTaskQueueEventTypes = ExtractEventTypes<
	typeof PqTestExecutableTaskQueueEvents
>;
...
export class PqTestExecutableTaskError extends Error {
	constructor(
		...
		super(
			processExit.stderr
				? processExit.stderr
				: `Failed to execute ${pqTestExeFullPath} ${formatArguments(
						processArgs
				  )}`
		);
...
export class PqTestExecutableDetailedTaskError extends Error {
	constructor(
		...
		super(
			`${details}, failed to execute ${pqTestExeFullPath} ${formatArguments(
				processArgs
			)}`
		);
		this.processExit = processExit;
...
export class PqTestExecutableTaskQueue implements IPQTestService, IDisposable {
	...
	constructor(
		...
		vscode.workspace.onDidSaveTextDocument(
			(textDocument: vscode.TextDocument) => {
				if (
					(textDocument.uri.fsPath.indexOf(".pq") > -1 &&
						textDocument.uri.fsPath.indexOf(".query.pq") === -1) ||
		...
		vscode.workspace.onDidCreateFiles((evt: vscode.FileCreateEvent) => {
			const filteredPaths: string[] = evt.files
				.filter(
					(oneUri: vscode.Uri) =>
						(oneUri.fsPath.indexOf(".pq") > -1 &&
							oneUri.fsPath.indexOf(".query.pq") === -1) ||
	...
	private resolvePQTestPath(
		...
		if (!nextPQTestLocation) {
			...
			this.outputChannel.appendErrorLine(
				resolveI18nTemplate(
					"PQSdk.taskQueue.error.pqtestLocationDoesntExist",
					{
						nextPQTestLocation,
					}
				)
		...
		const pqtestExe: string = path.resolve(
			nextPQTestLocation,
			PqTestExecutableTaskQueue.ExecutableName
		);

		if (!fs.existsSync(pqtestExe)) {
			this.outputChannel.appendErrorLine(
				resolveI18nTemplate(
					"PQSdk.taskQueue.error.pqtestExecutableDoesntExist",
					{
						pqtestExe,
					}
				)
	...
	private doSeizePQTestPidFromLockFile(
		thePidFileLocation: string = this.pidLockFileLocation
	): number | undefined {
		...
		if (typeof result === "number") {
			this.outputChannel.appendTraceLine(
				`Read existing pqTest pid file content: ${pidString}, ${result}`
			);
		} else {
			this.outputChannel.appendTraceLine(
				`Read non-existing pqTest pid file. ${result}`
			);
		}
	...
	private doSetupAndWatchPQTestExecutablePidLockFile(
		curPid: string = ""
	): void {
		...
		if (!fs.existsSync(this.pidLockFileLocation)) {
			...
			const pidNumber: number | undefined =
				this.doSeizePQTestPidFromLockFile();

			if (
				typeof pidNumber === "number" &&
				...
				this.outputChannel.appendTraceLine(
					`Find running pqTest of pid ${pidNumber}`
				);
			}
		}

		// unwatch the previous one when it got renamed
		if (this.onPQTestExecutablePidChangedFsWatcher)
			this.onPQTestExecutablePidChangedFsWatcher.close();
	...
	private async doCheckPidAndDequeueOneTaskIfAny(): Promise<void> {
		...
		if (typeof pidNumber !== "number" && this.pqTestFullPath) {
			...
			const pendingTask: PqTestExecutableTask | undefined =
			...
			if (pendingTask) {
				...
				const spawnProcess: SpawnedProcess = new SpawnedProcess(
					...
					{ cwd: this.pqTestLocation },
					{
						...
						onSpawned: (childProcess: ChildProcess): void => {
							...
							this.outputChannel.appendTraceLine(
								resolveI18nTemplate(
									"PQSdk.taskQueue.info.taskBegan",
									{
				...
				if (
					typeof processExit.exitCode === "number" &&
					...
					const errMessage: string =
						inferAnyGeneralErrorString(resultJson);
					...
					if (errMessage) {
						...
						if (pendingTask.operation === "info") {
							...
							pendingTask.operation === "list-credential"
						) {
					...
					this.outputChannel.appendErrorLine(
						resolveI18nTemplate(
							"PQSdk.taskQueue.info.taskExitAbnormally",
							{
								operation: pendingTask.operation,
								pid: `${spawnProcess.pid}`,
	...
	protected onPQTestExecutablePidChanged(
		...
		if (event === "change") {
			...
			if (typeof pidNumber === "number") {
				...
				this.eventBus.emit(
					PqTestExecutableTaskQueueEvents.processCreated
				);
			} else {
				this.eventBus.emit(
					PqTestExecutableTaskQueueEvents.processExited
				);
	...
	ExecuteBuildTaskAndAwaitIfNeeded(): Promise<void> {
		return executeBuildTaskAndAwaitIfNeeded(
			...
			(nextLastPqRelatedFileTouchedDate: Date) => {
				this.lastPqRelatedFileTouchedDate =
					nextLastPqRelatedFileTouchedDate;
			}
		);
	}
	...
	public DisplayExtensionInfo(): Promise<ExtensionInfo[]> {
		...
		return this.doEnqueueOneTask<any>({
			operation: "info",
			pathToConnector: resolveSubstitutedValues(
				ExtensionConfigurations.DefaultExtensionLocation
			),
		});
	}
	...
	public GenerateCredentialTemplate(): Promise<any[]> {
		...
		return this.doEnqueueOneTask<any>({
			...
			pathToQueryFile: resolveSubstitutedValues(
	...
	public SetCredentialFromCreateAuthState(
		...
		if (theAuthKind.toLowerCase() === "key") {
			...
			theAuthKind.toLowerCase() === "oauth" ||
			theAuthKind.toLowerCase() === "aad"
		) {
			additionalArgs.unshift("--interactive");
		} else if (
			theAuthKind.toLowerCase() === "implicit" ||
			theAuthKind.toLowerCase() === "anonymous"
		) {
	...
	public async RunTestBattery(pathToQueryFile: string = ""): Promise<any> {
		...
		if (
			!pathToQueryFile &&
			activeTextEditor?.document.languageId === "powerquery"
		) {
