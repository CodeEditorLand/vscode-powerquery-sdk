...
import {
	...
	READY,
} from "../pqTestConnector/PqServiceHostClientLite";
	...
	public static getTaskForPQTestTaskDefinition(
		...
		return new vscode.Task(
			...
			new vscode.CustomExecution(() =>
				Promise.resolve(new PqSdkTaskTerminal(taskDefinition))
			)
		);
	}
			...
			switch (this.taskDefinition.operation) {
				case "list-credential": {
					const result: unknown[] =
						await this.pqServiceHostClientLite.ListCredentials();
					...
					if (this.taskDefinition.credentialTemplate) {
						const setCredentialGenericResult: GenericResult =
							await this.pqServiceHostClientLite.SetCredential(
								JSON.stringify(
									this.taskDefinition.credentialTemplate
								)
							);
						...
						this.appendErrorLine(
							extensionI18n[
								"PQSdk.lifecycle.command.set.credentials.template.missing.errorMessage"
							]
						);
					}
				...
				case "run-test": {
					...
					const result: any =
						await this.pqServiceHostClientLite.RunTestBatteryFromContent(
							resolveSubstitutedValues(
								this.taskDefinition.pathToQueryFile
							)
						);
				...
				case "test-connection": {
					const testConnectionResult: GenericResult =
						await this.pqServiceHostClientLite.TestConnection();
