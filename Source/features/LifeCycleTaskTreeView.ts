...
const staticLifecycleTreeViewItem: LifecycleTreeViewItem[] = [
	...
	new LifecycleTreeViewItem(
		...
		{
			title: extensionI18n[
				"PQSdk.lifecycleTreeView.item.listCredentials.title"
			],
...
const staticSetupWorkspaceTreeViewItem: LifecycleTreeViewItem =
	new LifecycleTreeViewItem(
		extensionI18n["PQSdk.lifecycleTreeView.item.setupWorkspace.title"],
		{
			title: extensionI18n[
				"PQSdk.lifecycleTreeView.item.setupWorkspace.title"
			],
...
export class LifeCycleTaskTreeView
	implements TreeDataProvider<LifecycleTreeViewItem>
{
	...
	constructor(globalEventBus: GlobalEventBus) {
		...
		globalEventBus.on(
			GlobalEvents.workspaces.filesChangedAtWorkspace,
			(_args: any[]) => {
		...
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		globalEventBus.on(
			GlobalEvents.VSCodeEvents.ConfigDidChangePQTestExtension,
			(_args: any[]) => {
				this.debouncedRefresh();
			},
		);
		...
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		globalEventBus.on(
			GlobalEvents.VSCodeEvents.ConfigDidChangePQTestQuery,
			(_args: any[]) => {
				this.debouncedRefresh();
			},
		);
	...
	public debouncedRefresh: () => void = debounce(
	...
	async getChildren(
		element?: LifecycleTreeViewItem,
	): Promise<LifecycleTreeViewItem[] | undefined> {
		...
		if (await this.isValidWorkspace()) {
			...
			if (
				...
				const result: LifecycleTreeViewItem[] =
					staticLifecycleTreeViewItem.slice();
