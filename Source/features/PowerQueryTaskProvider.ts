...
const pqTestOperations: PowerQueryTaskDefinition[] = [
	...
	// TODO: Can we prompt to confirm that user really wants to remove all credentials?
	{
		...
		label: extensionI18n[
			"PQSdk.lifecycleTreeView.item.deleteAllCredentials.title"
		],
		additionalArgs: ["--ALL"],
	},
	{
		...
		operation: "info",
		label: extensionI18n[
			"PQSdk.lifecycleTreeView.item.displayExtensionInfo.title"
		],
	...
	// TODO: We need logic to determine which authentication kind to use (when there is more than one)
	{
		...
		label: extensionI18n[
			"PQSdk.lifecycleTreeView.item.createOneCredential.title"
		],
	...
	// TODO: This one should only be included for connectors with OAuth and Aad
	{
		...
		label: extensionI18n[
			"PQSdk.lifecycleTreeView.item.refreshCredentials.title"
		],
		pathToConnector: ExtensionConstants.ConfigPathToConnector,
		pathToQueryFile: ExtensionConstants.ConfigPathToTestConnectionFile,
	},
	// TODO: How can we format the output?
	{
		...
		operation: "run-test",
		label: extensionI18n[
			"PQSdk.lifecycleTreeView.item.evaluateOpenedFile.title"
		],
		pathToConnector: ExtensionConstants.ConfigPathToConnector,
	...
	},
	{
		...
		label: extensionI18n[
			"PQSdk.lifecycleTreeView.item.testConnection.title"
		],
		pathToConnector: ExtensionConstants.ConfigPathToConnector,
		pathToQueryFile: ExtensionConstants.ConfigPathToTestConnectionFile,
	},
...
export class PowerQueryTaskProvider implements vscode.TaskProvider {
	private static pendingTaskMap: WeakMap<vscode.Task, PendingTaskResolver> =
		new WeakMap();
	...
	public static executeTask(
		...
		return new Promise<vscode.TaskExecution>(
			(
				resolve: PendingTaskResolver["resolve"],
				reject: PendingTaskResolver["reject"],
			) => {
	...
	public provideTasks(
		...
		if (useServiceHost) {
			pqTestOperations.forEach((taskDef: PowerQueryTaskDefinition) => {
				result.push(
					PqSdkTaskTerminal.getTaskForPQTestTaskDefinition(taskDef),
				);
			});
		} else {
			pqTestOperations.forEach((taskDef: PowerQueryTaskDefinition) => {
				result.push(
					PowerQueryTaskProvider.getTaskForPQTestTaskDefinition(
						taskDef,
						this.pqTestService.pqTestFullPath,
					),
				);
			});
		}
	...
	public resolveTask(
		...
		const taskDef: PowerQueryTaskDefinition =
			task.definition as PowerQueryTaskDefinition;
		...
		if (taskDef.operation === "msbuild") {
			const msbuildFullPath: string | undefined =
				ExtensionConfigurations.msbuildPath;

			if (msbuildFullPath && !token.isCancellationRequested) {
				return PowerQueryTaskProvider.getTaskForBuildTaskDefinition(
					taskDef,
					msbuildFullPath,
				);
			}
		...
		if (taskDef.operation === "compile") {
			const currentWorkingFolder: string | undefined =
		...
		if (useServiceHost) {
			...
			return PowerQueryTaskProvider.getTaskForPQTestTaskDefinition(
				taskDef,
				pqtestExe,
			);
		}
	...
	private static getTaskForBuildTaskDefinition(
		...
		const execution: vscode.ProcessExecution = new vscode.ProcessExecution(
			msbuildExe,
		);
