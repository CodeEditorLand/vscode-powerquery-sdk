...
export module VscSettings {
	export async function findSettings(
		...
		const settingsEditor = workbench
	...
	export function findEnableServiceHostSetting(
		workbench?: Workbench
	): Promise<Setting> {
		// this string got rephrased out of the contributor path 'powerquery.sdk.features.useServiceHost'
		return VscSettings.findSettings(
			"Use Service Host",
			["Powerquery", "Sdk", "Features"],
			workbench
		);
	}

	export async function ensureUseServiceHostDisabled(
		workbench?: Workbench
	): Promise<void> {
		const enableServiceHostSetting =
			await VscSettings.findEnableServiceHostSetting(workbench);
		const enableServiceHostValue =
			await enableServiceHostSetting.getValue();
	...
	export async function ensureUseServiceHostEnabled(
		workbench?: Workbench
	): Promise<void> {
		const enableServiceHostSetting =
			await VscSettings.findEnableServiceHostSetting(workbench);
