...
export module VscSideBars {
	export async function expandAndShowPqSdkSection(
		workbench?: Workbench
	): Promise<ViewSection> {
		const primaryExplorerName = rootI18n["extension.pqtest.explorer.name"];

		const sideBarView = workbench
			? workbench.getSideBar()
			: new SideBarView();
		const pqSdkViewSection = await sideBarView
			.getContent()
			.getSection(primaryExplorerName);
	...
	export async function openFileFromDefaultViewSection(
		...
		const defaultViewSection = (await sideBarView
			.getContent()
			.getSection(workspaceName)) as DefaultTreeSection;
	...
	export async function clickClearAllCredentials(
		...
		const clearAllCredentialsItem = await pqSdkViewSection.findItem(
			deleteAllCredentialsTitle
		);
	...
	export async function clickSetCredentialAndPick(
		...
		const createOneCredentialTitle =
			extensionI18n[
				"PQSdk.lifecycleTreeView.item.createOneCredential.title"
			];
		const setCredentialItem = await pqSdkViewSection.findItem(
			createOneCredentialTitle
		);
