...
const MaximumPqTestNugetVersions: NugetVersions | undefined =
	MaximumPqTestNugetVersion
...
export module PqSdkNugetPackages {
	...
	export function getExpectedPqSdkToolPath(
		maybeNextVersion?: string
	): string | undefined {
		const pqTestSubPath: string[] = maybeNextVersion
			? buildPqSdkSubPath(maybeNextVersion)
			: PqTestSubPath;
	...
	export async function getAllPQSdkVersions(): Promise<NugetVersions[]> {
		const releasedVersions =
			await nugetHttpService.getSortedPackageReleasedVersions(
				PublicMsftPqSdkToolsNugetName,
				{
	...
	export async function assertPqSdkToolExisting(): Promise<void> {
		...
		if (!latestPQSdkNugetVersion) {
			const allNugetVersions: NugetVersions[] =
				await getAllPQSdkVersions();

			// eslint-disable-next-line require-atomic-updates
			latestPQSdkNugetVersion =
				allNugetVersions[allNugetVersions.length - 1];
		}

		const mayBeExpectedPQSDKToolExePath: string | undefined =
			getExpectedPqSdkToolPath(latestPQSdkNugetVersion.toString());

		expect(typeof mayBeExpectedPQSDKToolExePath).eq("string");

		const expectedPqTestExePath: string =
			mayBeExpectedPQSDKToolExePath as string;
