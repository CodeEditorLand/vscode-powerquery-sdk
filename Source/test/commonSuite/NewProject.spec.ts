...
describe("New extension project Tests", () => {
	describe("FirstConn project", () => {
		...
		it(`Command to create a new extension project ${newExtensionName} beneath ${oneTmpDir}`, async () => {
			...
			await ConnectorProjects.createOneNewExtensionProject(
				workbench,
				newExtensionName,
				oneTmpDir!
			);
			...
			ConnectorProjects.assertNewlyCreatedWorkspaceSettingsIntact(
				newExtensionName,
				oneTmpDir!
			);
			...
			const pqSdkViewSection =
				await VscSideBars.expandAndShowPqSdkSection(workbench);
			...
			await VscEditors.evalCurPqOfAnEditor(
				`${newExtensionName}.query.pq`,
				workbench
			);
			...
			const outputView =
				await VscOutputChannels.bringUpPQSdkOutputChannel();

			const currentPqSdkOutputText = await outputView.getText();
			expect(
				currentPqSdkOutputText.indexOf(
					`Hello from ${newExtensionName}: (no message)`
				)
			).gt(-1);
