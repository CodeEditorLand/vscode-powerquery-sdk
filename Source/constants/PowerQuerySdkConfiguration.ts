...
import {
	ExtensionConstants,
	PqModeType,
	SdkExternalsVersionTags,
} from "./PowerQuerySdkExtension";
...
export const ExtensionConfigurations = {
	...
	get httpProxyAuthorization(): string | undefined {
		...
		return config.get(
			ExtensionConstants.ConfigNames.http.proxyAuthorization
		);
	},
	get pqLocale(): string {
	...
	get pqMode(): PqModeType {
		...
		return config.get(
			ExtensionConstants.ConfigNames.PowerQuery.properties.mode
		) as PqModeType;
	},
	setPqMode(
		mode: PqModeType,
		configurationTarget:
			...
			| boolean
			| null = vscode.ConfigurationTarget.Global
	): Thenable<void> {
		const config: vscode.WorkspaceConfiguration =
			vscode.workspace.getConfiguration(
				ExtensionConstants.ConfigNames.PowerQuery.name
	...
	setMsbuildPath(
		...
		if (
			msbuildFullPath &&
			msbuildFullPath.indexOf(
				ExtensionConstants.MSBuildExecutableName
			) !== -1
		) {
	...
	setNugetPath(
		...
		if (
			nugetFullPath &&
			nugetFullPath.indexOf(ExtensionConstants.NugetExecutableName) !== -1
		) {
	...
	setDefaultExtensionLocation(
		...
		configurationTarget: vscode.ConfigurationTarget | boolean = vscode
	...
	get DefaultExtensionLocation(): string | undefined {
		...
		return (
			...
			config?.get(
				ExtensionConstants.ConfigNames.PowerQuerySdk.properties
					.deprecatedPqTestExtensionFileLocation
			)
		);
	},
	...
	get DefaultQueryFileLocation(): string | undefined {
		...
		return (
			...
			config?.get(
				ExtensionConstants.ConfigNames.PowerQuerySdk.properties
					.deprecatedPqTestQueryFileLocation
			)
		);
	},
