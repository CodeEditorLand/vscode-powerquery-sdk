...
const InternalMsftPqSdkToolsNugetName: string =
	"Microsoft.PowerQuery.SdkTools" as const;
...
function buildNugetPackageSubPath(
	packageName: string,
	version: string,
): string[] {
...
const MSBuildDownloadUrl: string =
	"https://visualstudio.microsoft.com/downloads/?q=build+tools" as const;

/**
 * It might be bash script like:
 *     `exec /usr/bin/memo /usr/lib/mono/nuget/Nuget.exe`
 */
const NugetExecutableName: string =
	os.type() === "Windows_NT" ? "Nuget.exe" : "nuget";
