...
export const GlobalEvents = Object.freeze({
	...
	VSCodeEvents: Object.freeze({
		...
		ConfigDidChangePowerQueryTestLocation:
			"ConfigDidChangePowerQueryTestLocation" as const,
		ConfigDidChangePQTestExtension:
			"ConfigDidChangePQTestExtension" as const,
		ConfigDidChangePQTestQuery: "ConfigDidChangePQTestQuery" as const,
		ConfigDidChangeExternalVersionTag:
			"ConfigDidChangeExternalVersionTag" as const,
...
export class GlobalEventBus
	extends DisposableEventEmitter<GlobalEventTypes>
	implements IDisposable
{
	...
	constructor(
		...
		vscode.workspace.onDidChangeWorkspaceFolders(
			(_e: WorkspaceFoldersChangeEvent) => {
				this.emit(
		...
		this.vscExtCtx.subscriptions.push(
			vscWorkspace.onDidChangeConfiguration(
				(evt: ConfigurationChangeEvent) => {
					if (
						...
						evt.affectsConfiguration(
							ExtensionConstants.ConfigNames.http.proxy
						) ||
						evt.affectsConfiguration(
							ExtensionConstants.ConfigNames.http
								.proxyAuthorization
						)
					) {
						this.emit(
