...
class MQueryConfigurationProvider implements vscode.DebugConfigurationProvider {
	...
	resolveDebugConfiguration(
		...
		if (!config.type && !config.request && !config.name) {
			const editor: TextEditor | undefined =
		...
		if (!config.program) {
			return vscode.window
				.showInformationMessage(
					extensionI18n["PQSdk.debugger.error.cannot.find.program"],
				)
				.then(
	...
	implements vscode.DebugAdapterDescriptorFactory
{
	createDebugAdapterDescriptor(
		...
		return new vscode.DebugAdapterInlineImplementation(
			new MQueryDebugSession(),
		);
	}
}

class MQueryNodeDebugAdapterNamedPipeServerDescriptorFactory
	implements vscode.DebugAdapterDescriptorFactory
{
...
export function activateMQueryDebug(
	...
	vscExtCtx.subscriptions.push(
		vscode.commands.registerCommand(
			...
			async (_config: unknown) => {
				const allPqTestFiles: vscode.Uri[] =
				...
				return vscode.window.showQuickPick(
					allPqTestFiles.map((oneUrl: vscode.Uri) =>
						vscode.workspace.asRelativePath(oneUrl, false),
					),
				);
			},
		),
	...
	const provider: MQueryConfigurationProvider =
		new MQueryConfigurationProvider();

	vscExtCtx.subscriptions.push(
		vscode.debug.registerDebugConfigurationProvider(
			ExtensionConstants.PQDebugType,
			provider,
		),
	);
