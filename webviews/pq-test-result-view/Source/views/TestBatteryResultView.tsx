...
export const TestBatteryResultView: React.FC<TestBatteryResult> =
	React.memo<TestBatteryResult>((props) => {
		...
		const DataSourceLabel = useI18n(
			"testBatteryResView.Table.Output.DataSource"
		);
		const ErrorLabel = useI18n("common.error.label");

		const hasOutput = useMemo(
			() =>
				Array.isArray(testRunExecution.Output) &&
				testRunExecution.Output.length,
			[testRunExecution]
		);
		...
		const errorDetailsString = useMemo<string | null>(() => {
			if (
				...
				testRunExecution.Status !== 3
			) {
				if (
					testRunExecution.Error?.Message &&
					typeof testRunExecution.Error?.Message === "string"
				) {
		...
		const hasDataSource = useMemo(
			() =>
				...
				testRunExecution.DataSourceAnalysis.length,
			[testRunExecution]
		);

		const summaryArr = useMemo(() => {
			const result: GeneralDetailItem[] = [];
			["ActivityId", "Method", "Name", "Status", "Type"].forEach(
				(key) => {
			...
			const startTime: Date = addFormattedDate(
				testRunExecution,
				"StartTime",
				result
			);
			const endTime: Date = addFormattedDate(
				testRunExecution,
				"EndTime",
				result
			);
		...
		const dataSourceArr = useMemo(() => {
			const result: GeneralDetailItem[] = [];
			if (hasDataSource) {
				const theFlattenDataSource = flattenJSON(
					testRunExecution.DataSourceAnalysis[0]
				);
		...
		return (
			<>
				...
				<Pivot
					aria-label="PQTest battery test result"
					defaultSelectedKey={hasOutput ? "Output" : "Summary"}>
					{hasOutput ? (
						<PivotItem key="output" headerText={OutputLabel}>
							<TestBatteryGeneralGrid
								items={testRunExecution.Output}
							/>
						</PivotItem>
					) : null}
