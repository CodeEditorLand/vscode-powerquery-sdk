...
const theVSCodeContextProps = React.createContext<VSCodeContextProps>(
	initVSCodeContextProps,
);

interface VSCodeContextActions {
	readonly onReady: () => void;
	readonly updateOneContextValue: (
		prop: string,
		value: any,
		alsoWriteToHist?: boolean,
	) => void;
}
...
const theVscodeContextActions = React.createContext<VSCodeContextActions>(
	initVSCodeContextActions,
);
...
export const VSCodeContextProvider: React.FC = React.memo((props) => {
	...
	const [curCtx, updateContext] = useState<VSCodeContextProps>(
		initVSCodeContextProps,
	);

	const updateOneContextValue = useCallback(
		(prop: string, value: any, alsoWriteToHist?: boolean) => {
			...
			switch (message.type) {
				case "OnOneValueUpdated":
					if (message.payload.property === "latestPqTestResult") {
						...
						message.payload.property === "activeColorTheme"
					) {
	...
	return (
		<theVSCodeContextProps.Provider value={curCtx}>
			<theVscodeContextActions.Provider value={actions}>
				{children}
			</theVscodeContextActions.Provider>
