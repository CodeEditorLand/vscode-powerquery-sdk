...
for (const oneLangId of allLangDirectories) {
	const currentLangIdDirectory: string = path.join(
		i18nDistDirectory,
		oneLangId,
	);
	...
	fs.copyFileSync(
		...
		path.join(
			webviewDistPqTestResI18nDirectory,
			`pq-test-result-view.${oneLangId}.json`,
		),
	);
...
const supportedLanguages: nls.Language[] = allLangDirectories.map(
	(id: string) => ({ id, folderName: id }),
);
