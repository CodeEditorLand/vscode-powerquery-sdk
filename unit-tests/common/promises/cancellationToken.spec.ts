...
} from "../../../src/common/promises/CancellationToken";
...
describe("Promises::CancellationTokenModule", () => {
	...
	describe("CancellationToken Class", () => {
		it("return the same token if any", () => {
			expect(CancellationToken.from(CancellationToken.none)).eq(
		...
		it("method: isCancelToken", () => {
			expect(CancellationToken.isCancellationToken(undefined as never))
				.false;
			expect(CancellationToken.isCancellationToken(null as never)).false;
			expect(CancellationToken.isCancellationToken({})).false;
			expect(
				CancellationToken.isCancellationToken(
					new CancellationToken(noop)
				)
			).true;
		});
	...
	describe("CancellationTokenSource Class", () => {
		...
		it("of dependents, papa canceling", () => {
			const { token, cancel }: CancellationTokenSource =
				new CancellationTokenSource();
			const { token: forked }: CancellationTokenSource =
		...
		it("of dependents, child canceling", () => {
			const { token }: CancellationTokenSource =
				new CancellationTokenSource();
			const { token: forked, cancel }: CancellationTokenSource =
				new CancellationTokenSource([token]);
			expect(token.requested).false;
			expect(forked.requested).false;
			cancel("Child reason");
