...
describe("NugetVersions.spec unit testes", () => {
	it("createFromReleasedVersionString v1", () => {
		const nugetVersions =
			NugetVersions.createFromReleasedVersionString("2.107.1");
	...
	it("createFromFuzzyVersionString v1", () => {
		const nugetVersions =
			NugetVersions.createFromFuzzyVersionString("2.107.1");
		expect(nugetVersions.major).eq("2");
		expect(nugetVersions.minor).eq("107");
		expect(nugetVersions.patch).eq("1");
	});
	...
	it("createFromFuzzyVersionString v2", () => {
		const nugetVersionsJunior =
			NugetVersions.createFromFuzzyVersionString("2.107.1");
		const nugetVersionsSenior =
			NugetVersions.createFromFuzzyVersionString("2.108.1");
		const nugetVersions =
			NugetVersions.createFromFuzzyVersionString("2.107.x");
		expect(nugetVersions.major).eq("2");
		expect(nugetVersions.minor).eq("107");
	...
	it("createFromFuzzyVersionString v3", () => {
		...
		const nugetVersionsSenior =
			NugetVersions.createFromFuzzyVersionString("2.108.1");
		const nugetVersions =
			NugetVersions.createFromFuzzyVersionString("2.107");
		expect(nugetVersions.major).eq("2");
		expect(nugetVersions.minor).eq("107");
	...
	it("createFromFuzzyVersionString v4", () => {
		const nugetVersionsJunior =
			NugetVersions.createFromFuzzyVersionString("2.108.1");
		const nugetVersionsSenior =
			NugetVersions.createFromFuzzyVersionString("3.108.1");
	...
	it("findClosestVersion v1", () => {
		let expectedNugetVersion =
			NugetVersions.createFromFuzzyVersionString("1.2.1");
		let closestNuGetVersion = NugetVersions.findClosetAmong(
			dummyNugetVersionsArrV1,
		...
		expectedNugetVersion =
			NugetVersions.createFromFuzzyVersionString("1.3.3");
		closestNuGetVersion = NugetVersions.findClosetAmong(
			dummyNugetVersionsArrV1,
			expectedNugetVersion
		);
		expect(closestNuGetVersion.major).eq("1");
	...
	it("findClosestVersion v2", () => {
		let expectedNugetVersion =
			NugetVersions.createFromFuzzyVersionString("2.114.4");
		let closestNuGetVersion = NugetVersions.findClosetAmong(
			dummyNugetVersionsArrV2,
			expectedNugetVersion
		);
		...
		expectedNugetVersion =
			NugetVersions.createFromFuzzyVersionString("2.1");
		closestNuGetVersion = NugetVersions.findClosetAmong(
			dummyNugetVersionsArrV2,
			expectedNugetVersion
		);
		...
		expectedNugetVersion =
			NugetVersions.createFromFuzzyVersionString("2.11");
		closestNuGetVersion = NugetVersions.findClosetAmong(
			dummyNugetVersionsArrV2,
			expectedNugetVersion
		);
		...
		expectedNugetVersion =
			NugetVersions.createFromFuzzyVersionString("2.110");
		closestNuGetVersion = NugetVersions.findClosetAmong(
			dummyNugetVersionsArrV2,
			expectedNugetVersion
		);
		...
		expectedNugetVersion =
			NugetVersions.createFromFuzzyVersionString("2.110.x");
		closestNuGetVersion = NugetVersions.findClosetAmong(
			dummyNugetVersionsArrV2,
			expectedNugetVersion
		);
		...
		expectedNugetVersion =
			NugetVersions.createFromFuzzyVersionString("2.110.4");
		closestNuGetVersion = NugetVersions.findClosetAmong(
			dummyNugetVersionsArrV2,
			expectedNugetVersion
		);
		...
		expectedNugetVersion =
			NugetVersions.createFromFuzzyVersionString("2.115");
		closestNuGetVersion = NugetVersions.findClosetAmong(
			dummyNugetVersionsArrV2,
			expectedNugetVersion
		);
		...
		expectedNugetVersion =
			NugetVersions.createFromFuzzyVersionString("2.116");
		closestNuGetVersion = NugetVersions.findClosetAmong(
			dummyNugetVersionsArrV2,
			expectedNugetVersion
		);
		...
		expectedNugetVersion =
			NugetVersions.createFromFuzzyVersionString("2.116.201");
		closestNuGetVersion = NugetVersions.findClosetAmong(
			dummyNugetVersionsArrV2,
			expectedNugetVersion
		);
	...
	it("filter out nuget version arr v1", () => {
		let maximumPqTestNugetVersion: NugetVersions =
			NugetVersions.createFromFuzzyVersionString("2.112.x");
		let minimumPqTestNugetVersion: NugetVersions =
			NugetVersions.createFromFuzzyVersionString("2.110.x");

		let filteredVersion = dummyNugetVersionsArrV2;

		filteredVersion = filteredVersion.filter(
			(one: NugetVersions) => one.compare(maximumPqTestNugetVersion) <= 0
		);

		filteredVersion = filteredVersion.filter(
			(one: NugetVersions) => minimumPqTestNugetVersion.compare(one) <= 0
		);
