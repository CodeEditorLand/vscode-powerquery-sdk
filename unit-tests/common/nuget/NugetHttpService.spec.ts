...
describe("NugetHttpService unit testes", () => {
	...
	it("downloadAndExtractNugetPackage v1", async () => {
		...
		await nugetHttpService.downloadAndExtractNugetPackage(
			SdkPackageName,
			theVersion,
			oneTmpDir
		);
		...
		expect(
			fs.existsSync(
				path.resolve(oneTmpDir, "Microsoft.PowerQuery.SdkTools.nuspec")
			)
		).true;
		expect(fs.existsSync(path.resolve(oneTmpDir, "tools", "PQTest.exe")))
			.true;
