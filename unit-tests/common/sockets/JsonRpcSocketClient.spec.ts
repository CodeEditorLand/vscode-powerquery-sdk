...
import {
	SocketAbortedConnection,
	SocketConnectionError,
} from "../../../src/common/sockets/SocketClient";
		...
		(): Promise<void> =>
			new Promise((resolve) => {
				server = new net.Server(
					...
					(socket: net.Socket) => {
						const reader: SocketMessageReader =
							new SocketMessageReader(socket, "utf-8");
						const writer: SocketMessageWriter =
							...
							if (Message.isRequest(message)) {
								if (message.method === "echoMethod") {
									void writer.write({
										id: message.id,
										result: (
											message.params as Array<unknown>
										)?.[0],
									} as ResponseMessage);
								}
